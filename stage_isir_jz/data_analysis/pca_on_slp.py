...
def plot_eof_patterns(ipca, shape_2d, outdir, n_eofs=10,wgts_flat = None,slp_std=1.0):
    ...
    for i in range(n_eofs):
        ...
        cbar = fig.colorbar(im, ax=axes[i], fraction=0.046, pad=0.04, shrink=0.6)
        cbar.set_label('SLP Anomaly (Pa)')
...
if __name__ == "__main__": 
    ...
    for member in val_members:
        ...
        # 1. Évaluation de la Loss totale
        ...
        # Variance totale (SLP) du membre autour de la moyenne globale du modèle
        X_val_centered = X_val - ipca.mean_
        slp_total_variance = np.sum(X_val_centered**2) 
        ...
        for k in range(1, ipca.n_components + 1):
            ...
            explained_var = 1.0 - (sse / slp_total_variance)
