...
# pas vraiment compatible avec le vae, en tout cas ne peut pas explorer tout un dossier de vae. Fixer la latent dim du vae si on en passe un (sinon je pense que ça bugue)
...
def objective(trial):
    ...
    print(f"test_members: {test_members}")
    # --- Lags ---
    ...
    # --- Dimension Latente Dynamique ---
    if args.latent_dim is not None:
        current_latent_dim = args.latent_dim
    else:
        # Optuna cherche la meilleure tronquature (jusqu'au max dispo dans le fichier PCA)
        current_latent_dim = trial.suggest_int("latent_dim", 1, max_latent_dim_target)
    
    trial.set_user_attr("current_latent_dim", current_latent_dim)

    # --- Hyperparamètres Linéaires ---
    input_format = args.input_format if args.input_format is not None else trial.suggest_categorical("input_format", ["raw", "pca"])
    sst_pca_dim = args.sst_pca_dim if args.sst_pca_dim is not None else (trial.suggest_int("sst_pca_dim", 1, 128) if input_format == "pca" else 0)
    ...
    # ============================================================
    # 3. INITIALISATION DU MODÈLE ET OPTIMIZER (ADAMW CORRIGÉ)
    # ============================================================
    if input_format == 'pca':
    ...
    out_features = current_latent_dim * len(quantiles) if loss_type == 'quantile' else current_latent_dim
    ...
    for epoch in range(args.nb_epochs):
        ...
        for batch_idx, (X_sst, X_slp, y_target, _, _, _) in enumerate(trainloader):
            ...
            # Cible latente avec tronquature dynamique (current_latent_dim)
            target_embed = encode_to_latent_gpu(y_target.to(device, non_blocking=True), args.embed_method, current_latent_dim, slp_pca_components_gpu[:current_latent_dim], slp_pca_mean_gpu, wgts_slp_gpu, vae_model)
            ...
            if (epoch == 0 and batch_idx in eval_steps_set) or (epoch == 1 and batch_idx in eval_steps_epoch2_set):
                ...
                # DÉCODAGE SPATIAL 
                val_pred_maps = decode_to_spatial_map_gpu(val_preds_latent, args.embed_method, slp_pca_components_gpu[:current_latent_dim], slp_pca_mean_gpu, wgts_slp_gpu, vae_model)
    ...
    test_pred_maps = decode_to_spatial_map_gpu(test_preds_latent, args.embed_method, slp_pca_components_gpu[:current_latent_dim], slp_pca_mean_gpu, wgts_slp_gpu, vae_model)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--optimize_metric', type=str, choices=['R2', 'L1', 'correlation'], default='R2', help="Métrique spatiale à maximiser")
    ...
    parser.add_argument('--latent_dim', type=int, default=None)  # DEFAULT = NONE -> Optuna décide
    ...
    parser.add_argument('--sst_pca_dim', type=int, default=None)
    ...
    if args.machine == 'hacienda': base_home = "/home/moysan/stage_isir_jz/linear_models/optuna_spatial/"
    elif args.machine in ['jean-zay-work', 'jean-zay-scratch']: base_home = "/lustre/fswork/projects/rech/uxg/uca57ub/stage_isir_jz/linear_models/optuna_spatial/"
    elif args.machine == 'mac_local': base_home = "/Users/remimoysan/Desktop/Jean_Zay/work_jz/stage_isir_jz/linear_models/optuna_spatial/"
    ...
    # Chargement Global de l'Embedder Target (SLP ou VAE)
    pca_model, vae_model = None, None
    slp_pca_mean_gpu, slp_pca_components_gpu = None, None
    max_latent_dim_target = 128

    if args.embed_method == 'pca':
        pca_model = joblib.load(args.embed_path) 
        max_latent_dim_target = pca_model.n_components_
        # CHARGEMENT COMPLET SUR GPU : On stocke tout, on tranchera dans la boucle
        slp_pca_mean_gpu = torch.tensor(pca_model.mean_, dtype=torch.float32, device=device)
        slp_pca_components_gpu = torch.tensor(pca_model.components_, dtype=torch.float32, device=device)
    elif args.embed_method == 'vae':
        match_dim = re.search(r'latent([0-9]+)', args.embed_path)
        if match_dim:
            max_latent_dim_target = int(match_dim.group(1))
        vae_model = ConvVAE(latent_dim=max_latent_dim_target).to(device)
        vae_model.load_state_dict(torch.load(args.embed_path, map_location=device))
        vae_model.eval()

    # Pondération Latitudes (SLP Target)
    wgts_slp_gpu = None
    if args.lat_weight:
    ...
    # Pondération Latitudes (SST Entrée)
    wgts_sst_gpu = None
    if args.lat_weight:
    ...
    # ============================================================
    base_name = f"study_{args.embed_method}_{args.optimize_metric}_m{''.join(map(str, args.winter_months))}_ep{args.nb_epochs}ie{args.nb_intra_evals}pat{args.patience}lag1{args.include_lag1}seq{args.sequential_lags}roll{args.roll_sst}latw{args.lat_weight}"
    short = {
        'nb_members_val': 'val', 'latent_dim': 'ldim', 'fixed_seed': 'seed', 'bs': 'bs', 'lr': 'lr','loss_type': 'loss', 'input_format': 'input',
    ...
    print(f"Début de l'optimisation LinReg SPATIALE ({args.n_trials} essais)...")
