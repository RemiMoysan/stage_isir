...
def plot_two_bootstrap_histograms(corr_tp_boot, corr_tt_boot, r2_tp_boot, original_corr, original_r2, month_name, pc_idx, member, outdir, freq_label):
    """Trace les histogrammes bootstrap avec la ligne de mesure."""
...
def bootstrap_r2(true_values, pred_values, n_iterations): 
    ...
    for i in range(n_iterations): 
        ...
        r2_scores[i] = 1 - (mse_boot / var_orig) 
...
def plot_time_series(pcs_true_dict, pcs_pred_dict, stats_dict, months_num, member, outdir, pc_idx, freq_label, quantiles_pred_dict=None):
    ...
    for i, m in enumerate(months_num):
        ...
        # Tracé des quantiles en dégradé
    ...
    fig.savefig(os.path.join(outdir, f'Perf_PC{pc_idx}_{freq_label}_Member_{member}.png'), dpi=300, bbox_inches='tight')
    plt.close(fig)
    ...
    fig_res.savefig(os.path.join(outdir, f'Residuals_PC{pc_idx}_{freq_label}_Member_{member}.png'), dpi=300, bbox_inches='tight')
    plt.close(fig_res)
