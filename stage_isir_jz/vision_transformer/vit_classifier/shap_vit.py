...
# IMPORT MODIFIÉ : On importe le ViT
from tools.models import ViT_Classifier_Multimodal
...
if __name__ == "__main__":
    ...
    # NOUVEAUX PARAMÈTRES SPÉCIFIQUES AU ViT
    parser.add_argument('--use_lags_attention', action='store_true', help='Activer l\'attention temporelle entre les lags du ViT')
    parser.add_argument('--embed_dim', type=int, default=128, help='Dimension de l\'embedding du ViT')
    ...
    # 1. INITIALISATION DU MODÈLE (ViT)
    model = ViT_Classifier_Multimodal(
        ...
        dr=0.0, # Dropout à 0 pour l'évaluation SHAP
        use_lags_attention=args.use_lags_attention                  
    ).to(device)

    # Note: Le ViT n'utilise pas de LazyLinear, donc pas besoin de dummy forward ici.
    ...
    explain_dir = os.path.join(outdir, f"explain_ViT_{args.method}_bg_{args.bg_type}_{args.n_test}samples_stride{args.beeswarm_stride}_val_members_{args.nb_members_val}")
    ...
    for c in range(args.num_classes):
        ...
        # 1. CARTE GLOBALE : Moyenne de la VALEUR ABSOLUE
        mean_abs_shap_sst = np.mean(np.abs(shap_sst_class), axis=0)
