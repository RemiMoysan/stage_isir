...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, members, selected_months, 
                 machine='jean-zay-work', target_type='map', num_pcs=10,
                 sst_lags_days=[35, 65, 95], slp_lags_days=[15], 
                 augment=False, custom_base_dir=None, duree_lissage=10, roll_sst=False,sst_std =0.707, slp_std=596):
        """
        Args:
...
class Dataset_mensuel(torch.utils.data.Dataset):
    def __init__(self, members, selected_months, 
                 ...
                 augment=False, custom_base_dir=None, roll_sst=False, 
        ...
        print("Ouverture des fichiers NetCDF mensuels (_1mo.nc) en cours...")
        for m in members:
            ...
            if self.target_type == 'pc':
                # Pense bien à vérifier que ce fichier existe avec ce nom exact
        ...
        print(f"Dataset_mensuel initialisé sur {machine.upper()}.")
        print(f"Target d'entraînement : {self.target_type.upper()}")
        print(f"Membres : {len(members)} | Mois valides par membre : {len(self.list_dates)}")
