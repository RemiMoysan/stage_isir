...
from tools.models import ViT_Latent_SLP_Multimodal, ConvVAE, vae_loss, compute_loss, get_median_prediction
...
if __name__ == "__main__":
    ...
    # Naming de l'output adapté
    if args.embed_method == 'pca':
        if not args.monthly_reduction:
            outdir_name = f"ViT_Latent_lags_attention_{args.use_lags_attention}_{args.embed_method}_normalize_{args.normalize}_lat_weight_{args.lat_weight}_loss_{loss_tag}_embedding_{latent_dim}_lags_{'_'.join(map(str, sst_lags_days))}_sst_{'_'.join(map(str, slp_lags_days))}_slp_bs{bs}_lr{lr}_dr{dr}_months_{'_'.join(map(str, winter_months))}_train{nb_members_train}_val_{nb_members_val}_{nb_members_test}_members_seed_{args.seed}_{duree_lissage}d_roll_sst_{args.roll_sst}"
        else:
            outdir_name = f"ViT_Latent_lags_attention_{args.use_lags_attention}_{args.embed_method}_normalize_{args.normalize}_lat_weight_{args.lat_weight}_loss_{loss_tag}_embedding_{latent_dim}_lags_{'_'.join(map(str, sst_lags_months))}_sst_{'_'.join(map(str, slp_lags_months))}_slp_bs{bs}_lr{lr}_dr{dr}_months_{'_'.join(map(str, winter_months))}_train{nb_members_train}_val_{nb_members_val}_{nb_members_test}_members_seed_{args.seed}_monthly_roll_sst_{args.roll_sst}"
    ...
    if args.embed_path:
        ...
        if match:
            ...
            print(f"\n✅ slp_std extrait du PCA : {dynamic_slp_std}")
        else:
            print(f"\n⚠️ 'slp_std' introuvable. Fallback : {dynamic_slp_std}")
    else:
        print(f"\n⚠️ Aucun modèle pré-entraîné. Fallback slp_std : {dynamic_slp_std}")
    ...
    # ============================================================
    # PRÉPARATION DES POIDS SPATIAUX (PCA PONDÉRÉ)
    # ============================================================
    wgts_flat = None
    if args.lat_weight and args.embed_method == 'pca':
        ...
        try:
            ...
            ds_sample.close()
            print("Grille de poids de latitude générée.")
        except Exception as e:
            print(f"Erreur chargement de la grille de latitude : {e}")
    ...
    print(f"-> Erreur RMSE moyenne de reconstruction (pas de prise en compte du weight): {rmse:.4f} hPa (environ)")
    ...
    model = ViT_Latent_SLP_Multimodal(
        sst_size=(85, 360), slp_size=(53, 113), patch_size_sst=(5, 10), patch_size_slp=(5, 10), in_chans_sst=len(active_sst_lags), in_chans_slp=len(active_slp_lags), embed_dim=128, depth=4, num_heads=4, dr=dr, nb_out=out_features, use_lags_attention=args.use_lags_attention
    ).to(device)
    ...
    # ============================================================
    # CALCUL DES STEPS DE VALIDATION INTRA-ÉPOQUE (Espacement Logarithmique)
    # ============================================================
    ...
    for epoch in range(nb_epochs):
        ...
        if epoch == 0 or epoch == 1:
            ...
            if args.nb_intra_evals > 0:
                ...
                if epoch == 1:
                    plot_correlation_evolution([],intra_epoch2_val_corrs, outdir, val_ks=intra_epoch2_val_ks, test_corrs = current_intra_test_corrs, test_ks=current_intra_test_ks, epoch_2 = True,batch_indexes = intra_epoch2_steps)
    ...
    # ---------------- VALIDATION ----------------
        ...
        for key in eval_phases:
                    ...
                    if key == 'val':
                        ...
                        if args.embed_method == 'pca':
                            ...
                            if args.lat_weight and 'safe_wgts' in locals():
                                ...
                                predicted_slp_flat, recon_true_slp_flat = predicted_slp_flat_polluted, recon_true_slp_flat_polluted
            ...
            # AJOUT : Calcul de v_k = std(pred) / std(target)
            k_vector = torch.sqrt(var_p / (var_t + 1e-8))
            k = k_vector.mean().item()

            val_ks.append(k) if key == 'val' else test_ks.append(k)
        ...
        if (epoch + 1) % 1 == 0:
            state = {'state_dict': model.state_dict(),
                    ...
                    'train_losses': train_losses, 'val_losses': val_losses, 'train_corrs': train_corrs, 'train_R2': train_R2, 'val_corrs': val_corrs, 'val_R2': val_R2, 'test_losses': test_losses, 'test_corrs': test_corrs, 'test_R2': test_R2,
                    'train_ks': train_ks, 'val_ks': val_ks, 'test_ks': test_ks} # <-- LIGNE À AJOUTER ICI
    ...
    eval_script_path = os.path.join(os.path.dirname(__file__), "eval_vit.py")
    ...
    for model_type in ["final", "best"]:
        for monthly_mean in [False, True]:
            ...
            if args.use_lags_attention:
                eval_command.append("--use_lags_attention")
            if monthly_mean:
            ...
            try:
                result = subprocess.run(eval_command, check=True, text=True, capture_output=True)
    ...
    eval_spatial_script_path = os.path.join(os.path.dirname(__file__), "eval_vit_full_slp.py")
    ...
    for model_type in ["final", "best"]:
        for monthly_mean in [False, True]:
            ...
            if args.use_lags_attention:
                eval_spatial_command.append("--use_lags_attention")
            if args.monthly_reduction:
            ...
            # Exécution
            try:
                result = subprocess.run(eval_spatial_command, check=True, text=True, capture_output=True)
