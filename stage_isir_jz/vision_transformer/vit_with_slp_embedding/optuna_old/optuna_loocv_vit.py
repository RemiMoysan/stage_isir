...
def objective(trial):
    ...
    transformer_act = args.transformer_act
    norm_first = args.norm_first
    use_lags_attention = args.use_lags_attention
    pool_strategy = args.pool_strategy
    head_act = args.head_act
    use_bottleneck = args.use_bottleneck
    head_hidden_dim = embed_dim // 2 if use_bottleneck else embed_dim # éventuellement rendre directement ça plus précis
    loss_type = args.loss_type
    sst_patch_y = args.sst_patch_y
    sst_patch_x = args.sst_patch_x
...
if __name__ == "__main__":
    ...
    parser.add_argument('--bs', type=int, default=32, help='Taille du batch')
    ...
    parser.add_argument('--depth', type=int, default=5, help='Profondeur du ViT')
    parser.add_argument('--num_heads', type=int, default=2, help='Nombre de têtes d\'attention dans le ViT')
    parser.add_argument('--head_dim', type=int, default=16, help='Dimension de chaque tête d\'attention dans le ViT')
    parser.add_argument('--mlp_ratio', type=float, default=6.0, help='Ratio MLP dans le ViT')
    parser.add_argument('--transformer_act', type=str, default='relu', help='Fonction d\'activation dans le ViT')
    parser.add_argument('--norm_first', action='store_true', help='Si activé, applique la normalisation avant l\'attention dans le Vit. Plutôt une bonne idée de l\'activer.')
    parser.add_argument('--use_lags_attention', action='store_true', help='Si activé, applique l\'attention sur les lags temporels dans le ViT. Plutôt désactivé')
    parser.add_argument('--pool_strategy', type=str, default='cls', choices=['cls', 'gap'], help='Stratégie de pooling')
    parser.add_argument('--head_act', type=str, default='relu', help='Fonction d\'activation dans la tête du ViT')
    parser.add_argument('--use_bottleneck', action='store_true', help='Si activé, utilise un goulot d\'étranglement dans la tête du ViT (plutôt à activer)')
    parser.add_argument('--loss_type', type=str, default='mse', choices=['mse', 'l1', 'correlation', 'quantile'], help='Type de loss (optionnel)')
    ...
    parser.add_argument('--sst_patch_y', type=int, default=17, help='Taille du patch vertical pour SST')
    parser.add_argument('--sst_patch_x', type=int, default=15, help='Taille du patch horizontal pour SST')
    ...
    base_home = "/lustre/fswork/projects/rech/uxg/uca57ub/stage_isir_jz/vision_transformer/vit_with_slp_embedding/optuna/loocv/"
    ...
    dynamic_name = f"LOOCV_metric_{args.optimize_metric}_{args.embed_method}{args.latent_dim}_months{''.join(map(str, args.winter_months))}_epochs{args.nb_epochs}_sst{''.join(map(str, args.sst_lags_months))}slp{''.join(map(str, args.slp_lags_months))}_pat{args.patience}intra{args.nb_intra_evals}bs{args.bs}dr{args.dr}att{args.use_lags_attention}lr{args.lr}roll{args.roll_sst}test{''.join(test_members)}loss{args.loss_type}depth{args.depth}head{args.num_heads}{args.head_dim}mlp{args.mlp_ratio}act{args.transformer_act}normfirst{args.norm_first}pool{args.pool_strategy}headact{args.head_act}bottleneck{args.use_bottleneck}weightdecay{args.weight_decay}_patchsst{args.sst_patch_y}x{args.sst_patch_x}"
    ...
    if not df.empty:
        ...
        print(f"\n=== Bilan LOOCV ViT ({args.optimize_metric.upper()} Optimisé) ===")
