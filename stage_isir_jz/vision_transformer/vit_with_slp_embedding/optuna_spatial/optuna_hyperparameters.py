...
def objective(trial):
    ...
    if args.embed_dir is not None and os.path.isdir(args.embed_dir):
        if args.embed_method == 'vae':
            ...
            if not parsed_models: raise ValueError("Aucun fichier VAE au format 'betaXlatentY.pth' trouvé.")
            ...
            if not matching_files: raise optuna.exceptions.TrialPruned()
    ...
    head_hidden_dim = max(int(embed_dim * bottleneck_ratio), current_latent_dim)
    ...
    sst_patch_x = args.sst_patch_x if args.sst_patch_x is not None else trial.suggest_categorical("sst_patch_x", [10, 15, 20, 30, 40]) 
    ...
    # ============================================================
    dynamic_slp_std = 466.93 
    match_std = re.search(r'slp_std([0-9.]+)', embed_path)
    if match_std: dynamic_slp_std = float(match_std.group(1))
    ...
    # ============================================================
    # 3. INITIALISATION DU MODÈLE ViT ET OPTIMIZER (ADAMW)
    # ============================================================
    ...
    try:
        ...
        if "Calculated output size" in str(e) or "Output size is too small" in str(e) or "out of memory" in str(e).lower() or "patch size" in str(e).lower():
            print(f"⚠️ Architecture physiquement impossible ou OOM. Trial élagué. Erreur: {e}")
    ...
    # --- TEST AVEC LE MEILLEUR MODÈLE (EVAL SPATIALE) ---
...
if __name__ == "__main__":
    ...
    parser.add_argument('--optimize_metric', type=str, choices=['R2', 'L1', 'correlation'], default='correlation', help="Métrique spatiale physique à maximiser par Optuna")
    ...
    base_name = f"study_vit_spatial_{args.embed_method}_{args.optimize_metric}_m{''.join(map(str, args.winter_months))}_ep{args.nb_epochs}ie{args.nb_intra_evals}pat{args.patience}val{args.nb_members_val}lag1{args.include_lag1}seq{args.sequential_lags}roll{args.roll_sst}latw{args.lat_weight}"
    short = {
        'latent_dim': 'ldim', 'fixed_seed': 'seed', 'bs': 'bs', 'lr': 'lr', 'dr': 'dr', 
        'depth': 'dp', 'num_heads': 'nh', 'head_dim': 'hd', 'mlp_ratio': 'mlp', 'transformer_act': 'tact', 
        'norm_first': 'nf', 'use_lags_attention': 'latt', 'pool_strategy': 'pool', 'head_act': 'hact', 
        'bottleneck_ratio': 'bn', 'loss_type': 'loss', 'sst_patch_y': 'py', 'sst_patch_x': 'px', 
        'weight_decay': 'wd', 'noise_std': 'ns', 'gradient_clip': 'gc', 
        'sst_lags_months': 'sstlags', 'slp_lags_months': 'slplags'
    }
    ...
    if args.machine == 'hacienda': base_home = "/home/moysan/stage_isir_jz/vision_transformer/vit_with_slp_embedding/optuna_spatial/"
    elif args.machine in ['jean-zay-work', 'jean-zay-scratch']: base_home = "/lustre/fswork/projects/rech/uxg/uca57ub/stage_isir_jz/vision_transformer/vit_with_slp_embedding/optuna_spatial/"
    elif args.machine == 'mac_local': base_home = "/Users/remimoysan/Desktop/Jean_Zay/work_jz/stage_isir_jz/vision_transformer/vit_with_slp_embedding/optuna_spatial/"

    output_dir = os.path.join(base_home, dynamic_name)
    os.makedirs(output_dir, exist_ok=True)
    db_path = os.path.join(output_dir, "vit_optuna_spatial.db")
    csv_path = os.path.join(output_dir, "vit_optuna_spatial_results.csv")
    ...
    print(f"Début de l'optimisation SPATIALE ViT (MAX {args.optimize_metric}) pour {args.n_trials} trials...")
    study.optimize(objective, n_trials=args.n_trials)
    
    print("\n=== Bilan HPO SPATIAL ViT ===")
