...
if __name__ == "__main__":
    ...
    if args.embed_method == 'pca':
        outdir_name = f"CNN_Latent_{args.embed_method}_normalize_{args.normalize}_embedding_{latent_dim}_lags_{'_'.join(map(str, sst_lags_days))}_sst_{'_'.join(map(str, slp_lags_days))}_slp_bs{bs}_lr{lr}_dr{dr}_months_{'_'.join(map(str, winter_months))}_train{nb_members_train}_val_{nb_members_val}_members_seed_{args.seed}_{duree_lissage}d"
    elif args.embed_method == 'vae':
        outdir_name = f"CNN_Latent_{args.embed_method}_beta_{args.beta_kld}_embedding_{latent_dim}_lags_{'_'.join(map(str, sst_lags_days))}_sst_{'_'.join(map(str, slp_lags_days))}_slp_bs{bs}_lr{lr}_dr{dr}_months_{'_'.join(map(str, winter_months))}_train{nb_members_train}_val_{nb_members_val}_members_seed_{args.seed}_{duree_lissage}d"
    ...
    model = CNN_Latent_SLP_Multimodal1(
        ...
        n_feat=8 # Tu peux l'augmenter à 16 ou 32 si tu as assez de VRAM
    ).to(device)
