...
# VAL_GRID_MEMBERS = ALL_MEMBERS[40:60]  
...
def objective(trial):
    ...
    # ============================================================
    # 4. BOUCLE D'ENTRAÎNEMENT ET TRACKING DES MÉTRIQUES
    # ============================================================
    ...
    # ============================================================
    # AJOUT : Calcul et stockage des scores par sample (pour TCL/Permutation)
    # ============================================================
    # Erreur quadratique moyenne (L2) par sample sur la dimension latente
    mse_per_sample = torch.mean((test_preds_latent - test_targets_latent)**2, dim=1)
    trial.set_user_attr("mse_per_sample", mse_per_sample.cpu().tolist())
...
if __name__ == "__main__":
    ...
    # ============================================================
    base_name = f"ComparisonNew_{args.embed_method}{args.latent_dim}_{args.optimize_metric}_m{''.join(map(str, args.winter_months))}"
    ...
    base_home = "/lustre/fswork/projects/rech/uxg/uca57ub/stage_isir_jz/cnn/cnn_with_slp_embedding/optuna_embedding/loocv_embedding/"
