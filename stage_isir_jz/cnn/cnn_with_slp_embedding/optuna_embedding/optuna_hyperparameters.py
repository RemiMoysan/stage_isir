...
def objective(trial):
    ...
    match_std = re.search(r'slp_std([0-9.]+)', args.embed_path)
    if match_std:
    ...
    trial.set_user_attr("embed_file", os.path.basename(args.embed_path))
    trial.set_user_attr("latent_dim", args.latent_dim)
    ...
    for epoch in range(args.nb_epochs):
        ...
        for batch_idx, (X_sst, X_slp, y_target, _, _, _) in enumerate(trainloader):
            ...
            target_embed = encode_to_latent_gpu(y_target.to(device, non_blocking=True), args.embed_method, latent_dim, pca_components_gpu, pca_mean_gpu, wgts_gpu, vae_model)
            ...
            if (epoch == 0 and batch_idx in eval_steps_set) or (epoch == 1 and batch_idx in eval_steps_epoch2_set):
                ...
                with torch.no_grad():
                    for v_X_sst, v_X_slp, v_y_target, _, _, _ in valloader_intra:
                        ...
                        v_target_embed = encode_to_latent_gpu(v_y_target.to(device, non_blocking=True), args.embed_method, latent_dim, pca_components_gpu, pca_mean_gpu, wgts_gpu, vae_model)
    ...
    print(f"Trial terminé en {time.time() - start_time:.2f} s | Latent Dim: {latent_dim} | Target Metric: {best_target_metric:.4f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--latent_dim', type=int, required=True, help="Dimension latente pour l'embedding (fixée pour ce script)")
    ...
    base_name = f"study_{args.embed_method}{args.latent_dim}_{args.optimize_metric}_m{''.join(map(str, args.winter_months))}_ep{args.nb_epochs}ie{args.nb_intra_evals}pat{args.patience}val{args.nb_members_val}lag1{args.include_lag1}seq{args.sequential_lags}roll{args.roll_sst}latw{args.lat_weight}"
    short = {
        'fixed_seed': 'seed', 'bs': 'bs', 'lr': 'lr', 'dr_conv': 'dr1', 'dr_fc': 'dr2', 'fc_dim': 'fc', 'depth': 'dp', 'n_feat': 'feat', 
    ...
    # --- CHARGEMENT DU MODÈLE D'EMBEDDING (Fixé pour ce script) ---
    ...
    base_home = f"/lustre/fswork/projects/rech/uxg/uca57ub/stage_isir_jz/cnn/cnn_with_slp_embedding/optuna_embedding/"
    output_dir = os.path.join(base_home, dynamic_name)
    os.makedirs(output_dir, exist_ok=True)
    db_path = os.path.join(output_dir, "cnn_optuna_embedding.db")
    csv_path = os.path.join(output_dir, "cnn_optuna_embedding_results.csv")
    ...
    print(f"Début de l'optimisation EMBEDDING MAX {args.optimize_metric}) pour {args.n_trials} trials...")
