...
# Il faut entrainer la PCA sinon OOM pour beaucoup de membres et évaluation impossible. 
...
if __name__ == "__main__":
    ...
    # À ajouter juste avant l'initialisation du modèle dans eval_cnn_full_slp.py
    ...
    model = CNN_Latent_SLP_Multimodal1(
        dr=0.0, 
        nb_out=out_features, 
        in_chans_sst=len(active_sst_lags), # C'est ici que le correctif s'opère
    ...
    cnn_path = os.path.join(args.cnn_dir, f"{args.model_type}_val_CNN_bs{args.bs}.pth")
    if not os.path.exists(cnn_path): cnn_path = os.path.join(args.cnn_dir, f"final_model_CNN_bs{args.bs}.pth")
    ...
    # ============================================================
    # 1. On concatène tout en gros tableaux Numpy
    ...
    for member in unique_members:
        ...
        # NOUVEAU : Définir et créer le dossier spécifique (val ou test)
        out_eval_dir = os.path.join(args.cnn_dir, f"spatial_{split_name}_{args.model_type}_{freq_label}")
        os.makedirs(out_eval_dir, exist_ok=True)

        # A. Isoler les données du membre courant
        mask = (members_arr == member)
        
        # B. Créer le Dataset Xarray
        ...
        # C. Application de la moyenne mensuelle AVANT de calculer l'erreur
        if args.monthly_mean:
            ds_member = ds_member.resample(time='1M').mean().dropna(dim="time")

        # D. Calcul des erreurs (au pixel) sur les données lissées
        ...
        # Pondération des erreurs pixel par pixel
        ...
        # E. Accumulation spatiale SÉPARÉE
        ...
        # F. Calcul et tracé de la Time Series pour ce membre (Moyenne sur C, H, W)
        ...
        plot_mse_timeseries(df_member_ts, member, out_eval_dir, freq_label,args.lat_weight) # J'ai remis out_eval_dir ici pour ne pas créer de sous-dossier
    ...
    # ============================================================
    # On moyenne sur le temps total et on enlève la dimension "Channel" (squeeze)

    for split in ['val', 'test']:
        if metrics[split]['time_steps'] == 0:
            continue # Sécurité si nb_members_test = 0

        out_eval_dir = os.path.join(args.cnn_dir, f"spatial_{split}_{args.model_type}_{freq_label}")
