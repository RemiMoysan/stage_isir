...
def objective(trial):
    ...
    nb_members_train = args.nb_members_train if args.nb_members_train is not None else trial.suggest_int("nb_members_train", 10, 60)
    ...
    if args.sst_lags_months is not None:
        ...
        if len(sst_lags_months) == 0:
            return float('inf') # Pire score possible (minimisation)
    ...
    mse_history = []
    trial.set_user_attr("mse_history", mse_history)
    ...
    with torch.no_grad():
        ...
        dummy_slp = None # (ou un tenseur de zéros si tu utilises des lags SLP)
    ...
    # ============================================================
    best_trial_mse = float('inf') # On cherche à MINIMISER
    patience_counter = 0
    ...
    for epoch in range(args.nb_epochs):
        ...
        for batch_idx, (X_sst, X_slp, y_target, _, _, _) in enumerate(trainloader):
            ...
            if (epoch == 0 and batch_idx in eval_steps_set) or (epoch == 1 and batch_idx in eval_steps_epoch2_set):
                # --- ÉVALUATION FIN D'ÉPOQUE (MSE et R2) ---
                model.eval()
                all_preds = []
                all_targets = []
                
                with torch.no_grad():
                    for X_sst, X_slp, y_target, _, _, _ in valloader_intra:
                        X_sst = X_sst.to(device, non_blocking=True)
                        X_slp = X_slp.to(device, non_blocking=True) if len(slp_lags_months) > 0 else None
                        ...
                        pred = model(X_sst, X_slp)
                        median_pred = get_median_prediction(pred, loss_type, quantiles, latent_dim) if loss_type == 'quantile' else pred
                        
                        all_preds.append(median_pred)
                        all_targets.append(target_embed)

                # Concaténer pour calculer les métriques globales sur tout le set de validation
                ...
                # 2. Calcul de la Variance de la Cible
                val_target_variance = torch.var(val_targets_tensor, unbiased=False).item()
                
                # 3. Calcul du R2 (Skill Score)
                epoch_r2 = 1.0 - (epoch_mse / val_target_variance) if val_target_variance > 0 else 0.0

                mse_history.append((epoch + batch_idx / total_batches, epoch_mse, epoch_r2))

                # Mise à jour des meilleurs scores
                if epoch_mse < best_trial_mse:
                    best_trial_mse = epoch_mse
                    best_r2_score = epoch_r2
                    patience_counter = 0
                else:
                    patience_counter += 1
        ...
        mse_history.append((epoch, epoch_mse, epoch_r2))

        # Enregistrer la variance cible de cet essai (elle dépend du split de validation)
        if epoch == 0:
        ...
        # PRUNING basé sur la MSE
        trial.report(epoch_mse, epoch)
        if trial.should_prune():
            trial.set_user_attr("mse_history", mse_history)
            raise optuna.exceptions.TrialPruned()

    # Enregistrer le meilleur R2 obtenu pendant cet essai
    trial.set_user_attr("best_r2_score", best_r2_score)
    trial.set_user_attr("best_trial_mse", best_trial_mse) # autant le save aussi non
    trial.set_user_attr("mse_history", mse_history)
    # On retourne la MSE (Optuna cherchera à la minimiser)
    return best_trial_mse
...
if __name__ == "__main__":
    ...
    parser.add_argument('--nb_members_train', type=int, default=None)
    ...
    fixed_params_str = f"_bs{args.bs}" if args.bs else ""
    fixed_params_str += f"_lr{args.lr}" if args.lr else ""
    dynamic_name = f"optuna_mse_min_months{''.join(map(str, args.winter_months))}{fixed_params_str}"
    ...
    # /!\ ATTENTION: direction="minimize" car on optimise la MSE
    ...
    print(f"Début de l'optimisation pour MINIMISER la MSE ({args.n_trials} trials)...")
    study.optimize(objective, n_trials=args.n_trials)
    
    print("\nMeilleur essai (Plus petite MSE) :")
    trial = study.best_trial
    print(f"  MSE: {trial.value:.4f}")
    print(f"  R2 Score (Skill Score): {trial.user_attrs.get('best_r2_score'):.4f}")
    print(f"  Variance de la Target pour cet essai: {trial.user_attrs.get('val_target_variance'):.4f}")
    print("  Paramètres :")
