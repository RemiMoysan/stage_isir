...
from shared_tools.evaluation_functions import plot_spatial_timeseries_raw_metrics, plot_metric_with_pvalue_map, compute_map_metrics_and_bootstraps
...
if __name__ == "__main__":
    ...
    # Arguments CNN Tunable
    parser.add_argument('--dr_conv', type=float, default=0.4); parser.add_argument('--dr_fc', type=float, default=0.1)
    parser.add_argument('--fc_dim', type=int, default=20); parser.add_argument('--n_feat', type=int, default=20)
    parser.add_argument('--activation', type=str, choices=['relu', 'tanh'], default='relu')
    parser.add_argument('--depth', type=int, default=3); parser.add_argument('--filter_mult', type=float, default=1.0)
    parser.add_argument('--pool_strategy', type=str, choices=['progressive','standart'], default='progressive')
    parser.add_argument('--pool_type', type=str, choices=['avg','max'], default='max')
    parser.add_argument('--sst_kx', type=int, default=3); parser.add_argument('--sst_ky', type=int, default=5)
    parser.add_argument('--sst_pool_x', type=int, default=2); parser.add_argument('--sst_pool_y', type=int, default=2)
    ...
    if args.embed_method == 'pca': pca_model = joblib.load(args.embed_path)
    elif args.embed_method == 'vae':
        vae_model = ConvVAE(latent_dim=args.latent_dim).to(device)
        vae_model.load_state_dict(torch.load(args.embed_path, map_location=device)); vae_model.eval()
    ...
    model = CNN_Latent_SLP_Multimodal1_tunable(
        ...
        in_chans_sst=len(active_sst_lags), in_chans_slp=len(active_slp_lags), n_feat=args.n_feat,
        early_fusion_sst=args.early_fusion_sst, depth=args.depth, filter_mult=args.filter_mult,
        sst_kx=args.sst_kx, sst_ky=args.sst_ky, sst_pool_x=args.sst_pool_x, sst_pool_y=args.sst_pool_y,
        pool_type=args.pool_type, pool_strategy=args.pool_strategy, activation=args.activation, use_gap=args.use_gap
    ).to(device)
    ...
    cnn_path = os.path.join(args.cnn_dir, f"best_val_CNN.pth" if args.model_type=='best' else "final_model_CNN.pth")
    if not os.path.exists(cnn_path):
        cnn_path = os.path.join(args.cnn_dir, f"best_val_CNN_bs{args.bs}.pth" if args.model_type=='best' else f"final_model_CNN_bs{args.bs}.pth")
    checkpoint = torch.load(cnn_path, map_location=device)
    model.load_state_dict(checkpoint.get('state_dict', checkpoint)); model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (X_sst, X_slp, y_target, y_map, dates, members) in enumerate(valloader):
            ...
            if len(y_target_np.shape) == 3: y_target_np = np.expand_dims(y_target_np, axis=1)
            ...
            if args.embed_method == 'pca':
                ...
                if args.lat_weight and 'safe_wgts' in locals(): slp_flat = slp_flat * wgts_flat
    ...
    for member in unique_members:
        ...
        member_base_dir = os.path.join(args.cnn_dir, f"spatial_eval_{args.model_type}", f"{member}_{split_name}", freq_label)
        ...
        for m in args.winter_months:
            ...
            # 1. Calcul des cartes & des 6 métriques spatiales
            ...
            # 2. Séries temporelles brutes (MSE, MAE, Corr) pour CE mois
            ...
            # 3. Tracé des 6 cartes (3 True + 3 Rec) avec p-values (vmax=0.30)
    ...
    print(f"\n✅ Évaluation spatiale par membre terminée avec succès ! Tous les graphiques sont dans : {args.cnn_dir}/spatial_eval")